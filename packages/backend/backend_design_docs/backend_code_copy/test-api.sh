...
run_tests() {
    ...
    if [ "$gql_code" = "200" ]; then
        if echo "$gql_body" | grep -q "UnauthorizedException"; then
            ...
            if echo "$gql_body" | grep -q "referrals\|payments"; then
                log_info "Available queries detected: referrals, payments, etc."
            fi
        fi
    else
    ...
    log_info "Cognito User Pool: us-west-2_42WkNSl7P"
    log_info "Cognito Client ID: 7agrl76dsiq2rabss3v6sn0r6u"
